...
(* Applique la stratégie d'un jouer sur la grille, renvoie true si l'action est autorisée *)
let do_move ((id, strategie) : player) (g : grid) = print_string ("Le joueur : " ^ (string_of_int id) ^ " joue " ^ fst (strategie g) ^" sur la grille \n"); true


(* Une Partie : Prend une liste de joueur et une grille de jeu et renvoie la liste des scores *)
